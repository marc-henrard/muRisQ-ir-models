...
import marc.henrard.risq.pricer.capfloor.RationalCapFloorProductPricer;
...
/**
 * Tests {@link RationalCapFloorLeastSquareCalibrator}.
 * 
 * @author Marc Henrard
 */
@Test
public class RationalCapFloorLeastSquareCalibratorTest {
  ...
  public void two_factor_smile_rat() {
    ...
    for (int k = 0; k < NB_STRIKES; k++) {
      ...
      MultiCurrencyAmount pvCheck = PRICER_TRADE.presentValue(trades.get(k), MULTICURVE, RATIONAL_2F);
      System.out.println(pvCheck);
    }
    ...
    RationalCapFloorLeastSquareCalibrator CALIBRATOR_RATIONAL = 
            RationalCapFloorLeastSquareCalibrator.of(template);
    RationalParameters calibrated = CALIBRATOR_RATIONAL.calibrate(trades, MULTICURVE, PRICER_TRADE);
    System.out.println(calibrated.getParameters());
...
//    fixed.set(5); // kappa
  ...
  /* Calibration at best of parameters b_0(0) and eta with a smile. Recover a arbitrary prices. */
...
//      System.out.println(pvLeg);
      ...
      trades.add(capTrade.resolve(REF_DATA));
      MultiCurrencyAmount pvCheck = PRICER_TRADE.presentValue(trades.get(k), MULTICURVE, RATIONAL_2F);
      sStart+= pvCheck.getAmount(EUR).getAmount() * pvCheck.getAmount(EUR).getAmount();
    }
    BitSet fixed = new BitSet(8);
    ...
    RationalTwoFactorHWShapePlusCstTemplate template =
        ...
        .of(RATIONAL_2F.getTimeMeasure(), RATIONAL_2F.getDiscountFactors(),
            RATIONAL_2F.getValuationTime(), RATIONAL_2F.getValuationZone(), 
//            RATIONAL_2F.getParameters(), fixed);
            DoubleArray.of(0.75, 0.50, 0.00, 0.50, 0.0099, 0.03, 0.00, 0.0020), fixed);
    RationalCapFloorLeastSquareCalibrator CALIBRATOR_RATIONAL = 
            RationalCapFloorLeastSquareCalibrator.of(template);
    RationalParameters calibrated = CALIBRATOR_RATIONAL.calibrate(trades, MULTICURVE, PRICER_TRADE);
    System.out.println(calibrated.getParameters());
    System.out.println("Root square error start: " + Math.sqrt(sStart));
    ...
    for (int i = 0; i < maturityIndex.length; i++) {
      ...
      for (int k = 0; k < NB_STRIKES; k++) {
        ...
        IborCapFloorTrade capTrade = IborCapFloorTrade.builder()
            ...
            .info(TradeInfo.of(VALUATION_DATE)).build();
        trades.add(capTrade.resolve(REF_DATA));
        MultiCurrencyAmount pvCheck = PRICER_TRADE.presentValue(trades.get(trades.size()-1), MULTICURVE, RATIONAL_2F);
        sStart+= pvCheck.getAmount(EUR).getAmount() * pvCheck.getAmount(EUR).getAmount();
      }
    }
    BitSet fixed = new BitSet(8);
...
//            RATIONAL_2F.getParameters(), fixed);
  ...
  private static void checkLeastSquare(
      ...
      RationalParameters calibrated,
      List<Integer> paramPerturbed) {
    
    double shift = 1.0E-3;
    double sCalibrated = 0.0;
    for (int looptrade = 0; looptrade < trades.size(); looptrade++) {
      MultiCurrencyAmount pvCheck = PRICER_TRADE.presentValue(trades.get(looptrade), MULTICURVE, calibrated);
      sCalibrated += pvCheck.getAmount(EUR).getAmount() * pvCheck.getAmount(EUR).getAmount();
      System.out.println(pvCheck);
    }
    System.out.println("Root square error calibrated: " + Math.sqrt(sCalibrated));
    for (int i = 0; i < paramPerturbed.size(); i++) {
      for (int loopside = 0; loopside < 2; loopside++) {
      RationalTwoFactorHWShapePlusCstParameters perturbed = (RationalTwoFactorHWShapePlusCstParameters) 
        calibrated.withParameter(paramPerturbed.get(i), 
            calibrated.getParameter(paramPerturbed.get(i)) + (2 * loopside - 1) * shift);
      double sPerturbed = 0.0;
      for (int looptrade = 0; looptrade < trades.size(); looptrade++) {
        MultiCurrencyAmount pvCheck = PRICER_TRADE.presentValue(trades.get(looptrade), MULTICURVE, perturbed);
        sPerturbed += pvCheck.getAmount(EUR).getAmount() * pvCheck.getAmount(EUR).getAmount();
      }
      System.out.println("Square error perturbed: " + Math.sqrt(sPerturbed));
