...
import marc.henrard.murisq.model.rationalmulticurve.RationalTwoFactor2HWShapeMultiplyParameters;
import marc.henrard.murisq.model.rationalmulticurve.RationalTwoFactorHWShapePlusCstParameters;

/**
 * Tests {@link RationalTwoFactor2HWShapeMultiplyParameters}.
 * 
 * @author Marc Henrard
 */
@Test
public class RationalTwoFactor2HWShapeMultiplyParametersTest {
  ...
  private static final RationalTwoFactor2HWShapeMultiplyParameters PARAMETERS_2HW =
      RationalTwoFactor2HWShapeMultiplyParameters.builder()
  ...
  public void of() {
    RationalTwoFactor2HWShapeMultiplyParameters test = RationalTwoFactor2HWShapeMultiplyParameters
  ...
  public void b0_1hw() {
    RationalTwoFactor2HWShapeMultiplyParameters param2hw =
        RationalTwoFactor2HWShapeMultiplyParameters.builder()
            .a1(A_1).a2(A_2).correlation(CORRELATION).b00(B_0_0).eta1(ETA_1).kappa1(KAPPA_1)
            .eta2(0.0).kappa2(KAPPA_1).c1(1.0).c2(0.0)
            .timeMeasure(TIME_MEASURE).discountFactors(DF).valuationTime(VAL_DATE_TIME.toLocalTime())
            .valuationZone(VAL_DATE_TIME.getZone()).build();
    RationalTwoFactorHWShapePlusCstParameters param1hw =
        RationalTwoFactorHWShapePlusCstParameters.builder()
            .a1(A_1).a2(A_2).correlation(CORRELATION).b00(B_0_0).eta(ETA_1).kappa(KAPPA_1).c1(0.0).c2(0.0)
            .timeMeasure(TIME_MEASURE).discountFactors(DF).valuationTime(VAL_DATE_TIME.toLocalTime())
            .valuationZone(VAL_DATE_TIME.getZone()).build();
  ...
  public void b1() {
    for (int loopdate = 0; loopdate < NB_TEST_DATES; loopdate++) {
      ...
      double b1Expected = 
          C_1 * (PARAMETERS_2HW.b0(obs.getEffectiveDate()) - PARAMETERS_2HW.b0(obs.getMaturityDate())) / delta;
  ...
  public void b2() {
    for (int loopdate = 0; loopdate < NB_TEST_DATES; loopdate++) {
      ...
      double delta = EUR_EURIBOR_3M.getDayCount().yearFraction(obs.getEffectiveDate(), obs.getMaturityDate());
      double b2Expected = 
          C_2 * (PARAMETERS_2HW.b0(obs.getEffectiveDate()) - PARAMETERS_2HW.b0(obs.getMaturityDate())) / delta;
  ...
  public void parameters_with() {
    ...
    for (int i = 0; i < PARAMETERS_2HW.getParameterCount(); i++) {
      RationalTwoFactor2HWShapeMultiplyParameters newParam = PARAMETERS_2HW.withParameter(i, test);
