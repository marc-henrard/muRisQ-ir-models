...
/**
 * This is a modified version of the original DiscountingFraProductPricer to fit with IBOR Fallback.
 * Valuation in the fixing period does not always work as it may be incoherent (but it is still the ISDA proposal).
 */
/**
...
public class DiscountingFraProductPricer {
   ...
   * <p>
   * The par rate is the fixed rate for which the FRA present value is 0.
   * 
   * @param fra  the product
   * @param provider  the rates provider
   * @return the par rate
  ...
  public double parRate(ResolvedFra fra, RatesProvider provider) {
    return forwardRate(fra, provider) + fra.getSpread();
  }
  ...
  //-------------------------------------------------------------------------
  /**
   * Calculates the currency exposure of the FRA product.
   * 
   * @param fra  the product
   * @param provider  the rates provider
   * @return the currency exposure of the product
   */
  public MultiCurrencyAmount currencyExposure(ResolvedFra fra, RatesProvider provider) {
    return MultiCurrencyAmount.of(presentValue(fra, provider));
  }
  ...
  private double unitAmountNone(ResolvedFra fra, RatesProvider provider) {
    ...
    return (forwardRate + fra.getSpread() - fixedRate) * yearFraction;
  }
  ...
  private double unitAmountIsda(ResolvedFra fra, RatesProvider provider) {
    ...
    double forwardRate = forwardRate(fra, provider);
    double yearFraction = fra.getYearFraction();
    return ((forwardRate + fra.getSpread() - fixedRate) / (1.0 + forwardRate * yearFraction)) * yearFraction;
  }

  // AFMA discounting method
  private double unitAmountAfma(ResolvedFra fra, RatesProvider provider) {
    double fixedRate = fra.getFixedRate();
    double forwardRate = forwardRate(fra, provider) + fra.getSpread();
    double yearFraction = fra.getYearFraction();
  ...
  private double derivativeIsda(ResolvedFra fra, RatesProvider provider) {
    ...
    return (1.0 + (fixedRate - fra.getSpread()) * yearFraction) * yearFraction * dsc * dsc; // fra.getSpread()
  }
