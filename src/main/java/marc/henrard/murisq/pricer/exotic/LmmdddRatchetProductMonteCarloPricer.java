...
public final class LmmdddRatchetProductMonteCarloPricer 
  ...
  public MulticurveEquivalentSchedule multicurveEquivalent(ResolvedSwap product) {
    ...
    for(SwapPaymentPeriod p: periods) {
      ...
      schedules.add(MulticurveEquivalent.of(ratchetPeriod.getIndex().calculateFixingDateTime(ratchetPeriod.getFixingDate()), 
          ImmutableList.of(NotionalExchange.of(Payment.of(ratchetPeriod.getCurrency(), 1.0d, ratePeriod.getPaymentDate()))),  // Amount in instrument?
  ...
  public double[][] aggregation( // path x cash flows
    ...
    int nbExpiries = me.getExpiriesCount();
    int nbFwdPeriods = model.getIborPeriodsCount();
    double[] delta = model.getAccrualFactors().toArrayUnsafe();
    double[] beta = model.getMultiplicativeSpreads().toArrayUnsafe();
    
    // 1. Coefficients as doubles
    ArgChecker.isTrue(product.getLegs().size()==1, "product must have one leg");
    ResolvedSwapLeg leg = product.getLegs().get(0);
    ArgChecker.isTrue(leg.getType().equals(SwapLegType.OTHER), "leg must be of type OTHER");
    ImmutableList<SwapPaymentPeriod> periods = leg.getPaymentPeriods();
    double[][] coefficientsMain = new double[nbExpiries][3];
    double[][] coefficientsFloor = new double[nbExpiries][3];
    double[][] coefficientsCap = new double[nbExpiries][3];
    double[] effectiveTimes = new double[nbExpiries];
    double[] paymentTimes = new double[nbExpiries];
    double[] amounts = new double[nbExpiries];
    for(int loopperiod=0; loopperiod<nbExpiries; loopperiod++) {
      SwapPaymentPeriod p=  periods.get(loopperiod);
      ArgChecker.isTrue(p instanceof RatePaymentPeriod, "payment periods must be of type RatePaymentPeriod");
      RatePaymentPeriod ratePeriod = (RatePaymentPeriod) p;
      ArgChecker.isTrue(ratePeriod.getAccrualPeriods().size()==1, "one accrual per payment period");
      ArgChecker.isTrue(ratePeriod.getAccrualPeriods().get(0).getRateComputation() instanceof IborRatchetRateComputation,
      ...
      RateAccrualPeriod accrualPeriod = ratePeriod.getAccrualPeriods().get(0);
      IborRatchetRateComputation ratchetPeriod = (IborRatchetRateComputation) accrualPeriod.getRateComputation();
      coefficientsMain[loopperiod] = ratchetPeriod.getMainCoefficients().toArrayUnsafe();
      coefficientsFloor[loopperiod] = ratchetPeriod.getFloorCoefficients().toArrayUnsafe();
      coefficientsCap[loopperiod] = ratchetPeriod.getCapCoefficients().toArrayUnsafe();
      effectiveTimes[loopperiod] = 
          model.getTimeMeasure().relativeTime(model.getValuationDate(), ratchetPeriod.getEffectiveDate());
      paymentTimes[loopperiod] = 
          model.getTimeMeasure().relativeTime(model.getValuationDate(), ratePeriod.getPaymentDate());
      amounts[loopperiod] = me.getSchedules().get(loopperiod).getIborPayments().get(0).getPaymentAmount().getAmount();
    }
    int[] indexIborTimes = model.getIborTimeIndex(effectiveTimes);
    int[] indexPaymentTimes = model.getIborTimeIndex(paymentTimes);

    double[][] pv = new double[nbPaths][nbExpiries];
    for (int looppath = 0; looppath < nbPaths; looppath++) { // loop paths
      double[] coupons = new double[nbExpiries + 1]; // one extra dim to facilitate recursion
      for (int loopexp = 0; loopexp < nbExpiries; loopexp++) { // loop expiries
        MulticurveEquivalentValues valuePathExpiry = valuesExpiries.get(looppath).get(loopexp);
        double[] valueFwdPathExpiry = valuePathExpiry.getOnRates().toArrayUnsafe();
        // generate df
        double[] discounting = new double[nbFwdPeriods + 1]; 
        // discounting. Note: spot to first Ibor date cancel between discounting and numeraire (also discounting)
        discounting[nbFwdPeriods] = 1.0; // discounting to the end of the period
        for (int loopdsc = nbFwdPeriods - 1; loopdsc >= 0; loopdsc--) {
          discounting[loopdsc] = discounting[loopdsc + 1] * (1.0 + valueFwdPathExpiry[loopdsc] * delta[loopdsc]);
        }
        // coupon value
        int i = indexIborTimes[loopexp];
        double iborRate = (beta[i] * (1 + delta[i] * valueFwdPathExpiry[i]) - 1.0d) / delta[i];
        double cpnMain = coefficientsMain[loopexp][0] * coupons[loopexp] 
            + coefficientsMain[loopexp][1] * iborRate 
            + coefficientsMain[loopexp][2];
        double cpnFloor = coefficientsFloor[loopexp][0] * coupons[loopexp] 
            + coefficientsFloor[loopexp][1] * iborRate 
            + coefficientsFloor[loopexp][2];
        double cpnCap = coefficientsCap[loopexp][0] * coupons[loopexp] 
            + coefficientsCap[loopexp][1] * iborRate 
            + coefficientsCap[loopexp][2];
        coupons[loopexp + 1] = Math.min(Math.max(cpnMain, cpnFloor), cpnCap);
        double a = amounts[loopexp];
        double c = coupons[loopexp + 1];
        double d = discounting[indexPaymentTimes[loopexp]];
        pv[looppath][loopexp] = a * c * d;
      } // end loop expiries
    } // end loop paths
    return pv;
  }
