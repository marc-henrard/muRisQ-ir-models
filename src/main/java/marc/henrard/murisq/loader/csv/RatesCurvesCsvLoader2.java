...
import com.opengamma.strata.loader.csv.RatesCurveGroupDefinitionCsvLoader;
...
/**
 * This is a copy of OpenGamma's Strata loader to solve method visibility.
 * 
 ...
 */
public final class RatesCurvesCsvLoader2 {
  ...
  private static Map<LocalDate, Map<CurveName, Curve>> parseCurves(
    ...
    // load curve settings
    Map<CurveName, LoadedCurveSettings2> settingsMap = parseCurveSettings(settingsResource);
  ...
  static Map<CurveName, LoadedCurveSettings2> parseCurveSettings(CharSource settingsResource) {
    ImmutableMap.Builder<CurveName, LoadedCurveSettings2> builder = ImmutableMap.builder();
    ...
    for (CsvRow row : csv.rows()) {
      ...
      LoadedCurveSettings2 settings =
          LoadedCurveSettings2.of(curveName, xValueType, yValueType, dayCount, interpolator, leftExtrap, rightExtrap);
  ...
  private static Multimap<LocalDate, Curve> parseSingle(
      ...
      Map<CurveName, LoadedCurveSettings2> settingsMap) {

    CsvFile csv = CsvFile.of(curvesResource, true);
    Map<LoadedCurveKey2, List<LoadedCurveNode2>> allNodes = new HashMap<>();
    for (CsvRow row : csv.rows()) {
      ...
      if (datePredicate.test(date)) {
        ...
        LoadedCurveKey2 key = LoadedCurveKey2.of(date, CurveName.of(curveNameStr));
        List<LoadedCurveNode2> curveNodes = allNodes.computeIfAbsent(key, k -> new ArrayList<>());
        curveNodes.add(LoadedCurveNode2.of(pointDate, pointValue, pointLabel));
      }
    }
  ...
  private static Multimap<LocalDate, Curve> buildCurves(
      Map<CurveName, LoadedCurveSettings2> settingsMap,
      Map<LoadedCurveKey2, List<LoadedCurveNode2>> allNodes) {

    ImmutableMultimap.Builder<LocalDate, Curve> results = ImmutableMultimap.builder();

    for (Map.Entry<LoadedCurveKey2, List<LoadedCurveNode2>> entry : allNodes.entrySet()) {
      LoadedCurveKey2 key = entry.getKey();
      LoadedCurveSettings2 settings = settingsMap.get(key.getCurveName());
